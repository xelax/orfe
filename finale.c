...
int cepitreopz (int orario,int indiceStazione,int verso,int opzioni)
{
	...
	i=j;
	while (i !=  ( j==0 ? tabcor.treni[verso]-1 : j-1) ) {
...
int StampaTrenino (lineaCapi *trenino,int trenini,int opzioni)
{
...
void stampa (Lampo *lampi, int opCliente)
{
	...
	for(trenini=0; (trenini < MAXTRENINI) && (orarLampi != NULL); orarLampi = orarLampi -> lamposeg) {
		itinerario++;
		SGR(GRASS,YELLW,40);
		printf("\n\n----------------------******* PERCORSO N. %d ******---------------------\n",itinerario);
		trenino = orarLampi -> spez;
		trenini = StampaTrenino(trenino,trenini,0);
		if (opCliente != 0){
			SGR(GRASS,MAGEN,40);
...
void stampaOpzioni (Sasso **tasca, int numeroPercorso)
{
	...
	int numeroIndiceTasca, pesoPartenza,pesoArrivo,totale;
	Sasso *punTasca;
	int prezzoCuccette = 0;
	int prezzoIntercity = 0;
	int prezzoEurocity = 0;
	int km = 0;
	...
	if(prezzoCuccette + prezzoIntercity + prezzoEurocity + prenotazione != 0) {
