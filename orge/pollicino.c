...
#define MAX_INP_TRATTI 20
/*----------------   GRAFO ---------------*/
#define MAXTRATTI (MAX_INP_TRATTI * 2)

extern int MaxTratti;
...
static Sasso *tasca1[MAXDIS];
static Sasso *tasca2[MAXDIS];
static Sasso *bisaccia[MAXDIS];
	...
	while ( (l =  cercaPesoMin(0)) != -1) {
		...
		for(indGraf=n; indGraf < ntratti; indGraf++) {
			...
			if ( (grafo[indGraf].x == pSassoCor->nodo) &&
				...
				for ( indBis = 0;
				      (indBis < MAXDIS-1) && (!(bisaccia[indBis] == NULL));
				      indBis++
				);
